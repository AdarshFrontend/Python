...
root = Tk()
root.title("A & S Product Info  Tech")
...
def Exit():
    result = tkMessageBox.askquestion('A&S Product info Tech', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("A&S Product info Tech/Account Login")
   
    loginform.geometry(f'{root.winfo_screenwidth()}x{root.winfo_screenheight()}')
    LoginForm()
...
def LoginForm():
    ...
    cursor = conn.cursor()
    global lbl_result
    TopLoginForm = Frame(loginform, width=600, height=100, bd=1, relief=SOLID,bg="lightblue")
    TopLoginForm.pack(side=TOP, pady=20)
    lbl_text = Label(TopLoginForm, text="Admin Login", font=('arial', 18), width=600,bg="red")
    ...
    btn_login = Button(MidLoginForm, text="Login", font=('arial', 18), width=30, command=Login,bg="red")
...
def Home():
    global Home
    Home = Tk()
    Home.title("A&S Product info Tech/Home")
    Home.geometry(f'{root.winfo_screenwidth()}x{root.winfo_screenheight()}')
    Title = Frame(Home, bd=1, relief=SOLID)
    Title.pack(pady=10)
    lbl_display = Label(Title, text=" Don't Cry Just Buy  ", font=('Elephant', 45),fg="red")
    lbl_display.pack()
    menubar = Menu(Home)
    ...
    menubar.add_cascade(label="Product/Prices", menu=filemenu2)
...
def ShowAddNew():
    global addnewform
    addnewform = Toplevel()
    addnewform.title("A&S Product info Tech/Add new")
    addnewform.geometry(f'{root.winfo_screenwidth()}x{root.winfo_screenheight()}')
    AddNewForm()

def AddNewForm():
    TopAddNew = Frame(addnewform, width=600, height=100, bd=1, relief=SOLID)
    TopAddNew.pack(side=TOP, pady=20)
    lbl_text = Label(TopAddNew, text="Add New Product", font=('Elephant', 18), width=600,bg="red")
    lbl_text.pack(fill=X)
...
def ViewForm():
    ...
    lbl_text = Label(TopViewForm, text="View Products", font=('arial', 18), width=600,bg="red")
    lbl_text.pack(fill=X)
    lbl_txtsearch = Label(LeftViewForm, text="Search", font=('arial', 20))
    lbl_txtsearch.pack(side=TOP, anchor=W)
    search = Entry(LeftViewForm, textvariable=SEARCH, font=('arial', 15), width=10)
    search.pack(side=TOP,  padx=10, fill=X)
    btn_search = Button(LeftViewForm, text="Search", command=Search,bg="red")
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('A&S Product info Tech', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("A&S Product info Tech/View Product")
    viewform.geometry('600x400')
    ViewForm()

def Logout():
    result = tkMessageBox.askquestion('A&S Product info Tech', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes': 
        admin_id = ""
        root.deiconify()
        Home.destroy()
...
l=Label(image=img,width= 800,height=400)
l.pack()
Title.pack(pady=10)

lbl_display = Label(Title, text="A&S Product info Tech", font=('Elephant', 45),fg="red")
lbl_display.pack()
root.mainloop()
